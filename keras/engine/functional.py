...
class Functional(training_lib.Model):
  ...
  def __init__(self, inputs, outputs, name=None, trainable=True,
    ...
    # Check if the inputs contains any intermediate `KerasTensor` (not directly
    # from tf.keras.Inputs()). We need to clone the `Node` and `KerasTensor` in
    # this case to mimic user rebuild a new model from a new input.
    # This feature is only enabled in tf2+tf.function and not in v1 graph mode.
